...
if [ "$1" = "jmx" ]; then
    ...
    if [ -n "$IP" ]; then
        echo "The $SERVER_NAME IP is $IP"
    else
        IP='192.168.85.129'
    fi
    JAVA_JMX_OPTS=" -Djava.rmi.server.hostname=$IP -Dcom.sun.management.jmxremote.port=1099 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false "
fi
